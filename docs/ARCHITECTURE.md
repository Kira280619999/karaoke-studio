...
- `Separator`: Mel-Band RoFormer, HTDemucs FT and center-cancel remain analysis inputs. BS PolarFormer 62-band FP32 ONNX runs as an export-only candidate, is excluded from lyric alignment, and becomes the default Final stem when its WAV pair is available. Its pinned 201 MiB checkpoint is downloaded atomically and accepted only after size and SHA-256 validation. PolarFormer uses a constant-memory streaming path: four-second FP32 windows, one-second equal-power overlap, incremental float32 WAV writes, sequential ONNX execution, and disabled ONNX CPU arena/memory patterns. This replaces the upstream 20-second window that can exceed 10 GiB RSS; the verified Apple-Silicon peak for both 12-second and 30-second real-audio runs is about 3.24 GiB. ViperX 1297 is attempted only as a fallback after a PolarFormer failure.
