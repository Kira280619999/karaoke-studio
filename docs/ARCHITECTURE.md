...
- `Separator`: Mel-Band RoFormer, HTDemucs FT and center-cancel remain analysis inputs. BS PolarFormer 62-band FP32 ONNX runs as an export-only candidate, is excluded from lyric alignment, and becomes the default Final stem when its WAV pair is available. Its pinned 201 MiB checkpoint is downloaded atomically and accepted only after size and SHA-256 validation. ViperX 1297 is attempted only as a fallback after a PolarFormer failure.
