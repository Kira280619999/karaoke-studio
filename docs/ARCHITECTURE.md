...
- `Separator`: Mel-Band RoFormer, HTDemucs FT and center-cancel remain analysis inputs. BS PolarFormer 62-band FP32 ONNX runs as an export-only candidate, is excluded from lyric alignment, and becomes the default Final stem when its WAV pair is available. Its pinned 201 MiB checkpoint is downloaded atomically and accepted only after size and SHA-256 validation. PolarFormer uses a constant-memory streaming path: fixed six-second FP32 windows, 500 ms equal-power overlap, incremental float32 WAV writes, sequential ONNX execution, static `batch`/`time_frames` dimension overrides, adaptive intra-op threads capped at ten, and disabled ONNX CPU arena/memory patterns. This replaces the upstream 20-second window that can exceed 10 GiB RSS. A 30-second real-audio comparison completed in 22.68 seconds with about 4.98 GiB peak model-process RSS, reconstructed the mix at 151.93 dB SNR and matched the v0.1.3 instrumental at 110.86 dB SNR. CoreML EP is deliberately rejected for the 24-GB target because this graph split into 125 CoreML partitions and peaked near 19 GiB even with a compiled cache; native WebGPU is also rejected until its output-readback path is correct and non-blocking. ViperX 1297 is attempted only as a fallback after a PolarFormer failure.
...
During review the Viewer defaults to `work/mix.wav`, preserving the singer for accurate cue decisions, and can switch to the prepared PolarFormer stem at the same playhead. Separation is non-destructive and prepares candidates only. PolarFormer is auto-selected and confirmed only after both of its WAV stems exist; if it fails, the same rule applies to the ViperX fallback. Timing review affects the quality label, not permission to render. Manual Precision intentionally does not call AI: selecting the first token loops the tail of the previous line through the new onset, and the operator controls start/end handles directly. The diagnostics suggestion endpoint remains read-only and detached from this manual workflow.
