...
- `Separator`: Mel-Band RoFormer, HTDemucs FT and center-cancel remain analysis inputs. BS-RoFormer ViperX 1297 runs as an export-only candidate, is excluded from lyric alignment, and becomes the single default Final stem when its WAV pair is available.
