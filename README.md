...
- Pipeline phân tích giữ nguyên Mel-Band RoFormer, `htdemucs_ft` và center-cancel fallback; BS-RoFormer ViperX 1297 được chạy riêng làm stem Karaoke mặc định cho Preview/Final và không thay đổi kiến trúc căn lời đang ổn định.
...
- Khung Preview lấy mẫu trên lưới 1920×1080/120fps để timecode, bước từng frame, chuyển động nền và quét chữ vẫn chính xác đến 1/120 giây; bản xuất 60fps tương thích lấy đúng các mốc khung tương ứng. Viewer luôn mặc định phát `GỐC` để kiểm tra lời và nhịp trên audio nguồn; nút `KARAOKE / GỐC` đổi sang ViperX ở đúng playhead mà không làm lệch hình. Waveform đổi theo đúng nguồn đang nghe. Số khung thực sự nhìn thấy phụ thuộc tần số quét màn hình. Preview chỉ phát video và hai dòng Karaoke, không có vùng bấm theo từ, tay nắm timing hoặc toolbar nổi che hình. Mọi chỉnh sửa được tập trung duy nhất ở `Cuộn timeline toàn bài`; khối Timing Editor trùng chức năng đã được loại bỏ.
- Bản có ca sĩ luôn dùng mix gốc và có watermark `TIMING NOT VERIFIED` khi timing chưa được xác nhận. Khi ViperX đã tạo thành công, app tự đặt đúng stem này làm Karaoke mặc định để xuất Final; điểm timing chưa duyệt vẫn là cảnh báo và được ghi rõ trong tên file cùng `QA_REPORT.json`, không khóa Final.
...
Stem Final mặc định dùng checkpoint `model_bs_roformer_ep_317_sdr_12.9755.ckpt` (BS-RoFormer ViperX 1297) thông qua [Audio Separator](https://github.com/nomadkaraoke/python-audio-separator). Checkpoint được Audio Separator tải tự động vào thư mục model local trong lần phân tích đầu tiên, được checksum vào manifest dự án và không được đóng gói lại trong GitHub Release.
...
5. Sau khi timing ổn, chuyển `GỐC / KARAOKE` tại cùng playhead nếu muốn đối chiếu. ViperX 1297 là candidate Final duy nhất và được đặt mặc định khi stem tạo thành công.
6. Có thể xuất bản có ca sĩ bằng mix gốc bất cứ lúc nào hoặc xuất bản loại giọng bằng ViperX. Timing còn cần duyệt vẫn không khóa xuất và được ghi vào QA. Khi hàng chờ đã hết và toàn bộ timing cũng đạt, project chuyển `VERIFIED`.
...
Open `http://127.0.0.1:3000`. No user media is sent to a cloud service. Review and singer-reference exports keep the original singer and are visibly watermarked. The Viewer defaults to the original mix and offers an explicit Original/Karaoke toggle at the same playhead. BS-RoFormer ViperX 1297 is prepared separately as the single default Final stem and is excluded from lyric alignment so the established timing architecture remains unchanged. Pending timing review points remain advisory and are recorded in the QA report instead of locking export. Optional model weights are downloaded locally and are never committed to this repository.
